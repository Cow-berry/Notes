...
\begin{example}
    ...
    $\Omega = \mathsds{B}^6\quad p(G) = \frac{1}{64}$
...
\begin{definition}
    ...
    $\left[ 7xi = i \right]  = \{\omega |  \xi(\omega) = i\}\subseteq \Omega$
...
\begin{figure}[ht]
    ...
    \caption{непрерывная вероятность}
...
\begin{proof}
     \begin{align*}
         E_{\xi} &= \sum_{\omega}p\left( \omega \right) \xi\left( \omega \right)  \\
                 ...
                 &= \sum_i\sum_{w:\xi\left( \omega \right) =i} p\left( \omega \right) \\
...
\begin{proof}
    ...
    $i\cdot j = \alpha\quad i\n R_{\xi}\quad j\in R_{\eta}$
\end{proof}
